...
from mock.mock_datas import user_details, users, product_details, products, orders, comments, user_credentials

# 创建web app
app = FastAPI(
    title="API Mock Server",
    description="这是一个用于演示api的Mock服务器，提供了各种类型的API接口示例。",
    version="1.0.0",
    docs_url="/api/docs",
    redoc_url="/api/redoc",
...
# 用于定义 OAuth2 密码模式（Resource Owner Password Credentials） 的一个安全机制实例。
# 它的作用是让 FastAPI 知道用户应该通过哪个 URL 获取访问令牌（token），并在需要认证的接口中自动从请求头中提取 Authorization: Bearer <token> 字段。
...
# 获取当前用户
async def get_current_user(token_data: TokenData = Depends(verify_token)):  # Depends用于设置依赖函数，并自动执行
    """根据令牌获取当前用户"""
    for user_cred in user_credentials:
...
async def login_for_access_token(login_data: LoginRequest):
    ...
    access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)  # 设置时间差
...
class AuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        auth_whitelist = [
            ...
            "/api/docs",  # Swagger文档
            "/api/redoc",  # ReDoc文档
            "/api/api-openapi.json",  # OpenAPI规范
        ]
